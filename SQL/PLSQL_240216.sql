...
-- 예외처리
-- 1) Oracle이 관리하고 있고, 예외 이름이 존재하는 경우 (대략 20가지 정도 됨.)
DECLARE
...
EXCEPTION
    WHEN e_emps_remaining THEN
        DBMS_OUTPUT.PUT_LINE('다른 테이블에서 사용중');
END;
/
...
-- IN OUT 매개변수(전달한 값이 덮어쓰여짐. 하나의 변수에 값을 덮어쓰는 형식. IN과IN OUT의 차이.)

-- '01012341234' 를 '010-1234-1234'로 변경하기 를 IN OUT의 예제로 많이 쓰임
-- 넘기는건 숫자라도 데이터타입이 넘버면 제일 앞자리 0이 날라감. 데이터 타입이 뭐냐에 따라 값이 달라짐. 넘버에서는 제일 앞자리 0이 필요없기때문.
CREATE PROCEDURE format_phone
(p_phone_no IN OUT VARCHAR2)
IS

BEGIN
    p_phone_no := SUBSTR(p_phone_no, 1, 3) -- 첫번쨰 기준으로 세글자 잘라냄 ( 음수를 사용하면 뒤에서부터 잘라냄)
                    || '-' || SUBSTR(p_phone_no, 4, 4) -- 네번째부터 네글자 잘라냄
                    || '-' || SUBSTR(p_phone_no, 8);  -- 여덟번째부터 반환
END;
/
